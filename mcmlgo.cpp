 ...
 ****/
void InitializePhotonPacket(const double Rspecular, LayerStruct* Layerspecs_Ptr, PhotonStruct* Photon_Ptr)
{
	Photon_Ptr->w = 1.0 - Rspecular;
	Photon_Ptr->dead = 0;
	Photon_Ptr->layer = 1;
	Photon_Ptr->step_size = 0;
	Photon_Ptr->sleft = 0;
...
void HopInGlass(InputStruct* In_Ptr, PhotonStruct* Photon_Ptr, OutStruct& Out_Ptr)
{
	...
	else
	{
		...
		Hop(Photon_Ptr); // Move the photon s away in the current layer of medium. 
