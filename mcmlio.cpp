 ...
 ****/
char* CenterStr(short  Wid, const char* InStr, char* OutStr)
{
	size_t nspaces;	/* number of spaces to be filled before InStr. */
...
void ReadDzDr(FILE* File_Ptr, InputStruct* In_Ptr)
{
	...
	if (In_Ptr->dz <= 0)
	{
		nrerror("Nonpositive dz.\n");
	}

	if (In_Ptr->dr <= 0)
	{
		nrerror("Nonpositive dr.\n");
	}
}
 ...
 ****/
void ReadAmbient(FILE* File_Ptr, LayerStruct* Layer_Ptr, const char* side)
{
	double n;
...
void CheckParm(FILE* File_Ptr, InputStruct* In_Ptr)
{
	...
	NameLink head = NULL;
	bool name_taken;/* output files share the same */
					  /* file name.*/
	char msg[STRLEN];
 ...
 *	ASCII or binary.
 ****/
void WriteVersion(FILE* file, const char* Version)
{
	fprintf(file,
...
void WriteInParm(FILE* file, InputStruct In_Parm)
{
	...
	for (i = 1; i <= In_Parm.num_layers; i++) {
		...
		fprintf(file, "%G\t%G\t%G\t%G\t%G\t# layer %hd\n",
			s.n, s.mua, s.mus, s.anisotropy, s.z1 - s.z0, i);
	}
